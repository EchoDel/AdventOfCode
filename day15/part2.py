...
final_grid = np.zeros((len(input[0]) * times_bigger, len(input) * times_bigger))
...
# Make the grid x times bigger
grid_size = (len(input), len(input[0]))

for x_times in range(times_bigger):
    for y_times in range(times_bigger):
        for x in range(grid_size[0]):
            for y in range(grid_size[1]):
                if (grid[x, y] + x_times + y_times) > 9:
                    final_grid[grid_size[0] * x_times + x, grid_size[1] * y_times + y] = (grid[x, y] + x_times + y_times) % 9
                else:
                    final_grid[grid_size[0] * x_times + x, grid_size[1] * y_times + y] = (grid[x, y] + x_times + y_times)
...
for r, line in enumerate(final_grid):
    for c, char in enumerate(final_grid):
        graph.add_node((r, c))


for r, line in enumerate(final_grid):
    for c, char in enumerate(line):
        for r1, c1 in grid_adjs((r, c), ((0, len(final_grid)), (0, len(final_grid[0])))):
            graph.add_edge((r, c), (r1, c1), weight=final_grid[r1, c1])

print(nx.shortest_path(graph, (0, 0), (len(final_grid) - 1, len(final_grid[0]) - 1), 'weight'))
print(nx.shortest_path_length(graph, (0, 0), (len(final_grid) - 1, len(final_grid[0]) - 1), 'weight'))
